...
/** Rows per "Sækja eldri" step. */
const PAGE_SIZE = 50;

/**
 * /tilkynningar — full list, newest first, unread emphasised. Click → mark read
 * + navigate to the link. "Sækja eldri" widens the window (`listNotifications`
 * only takes a limit); the rows already on screen stay put while the wider
 * page loads. The realtime channel is mounted once by AppShell
...
export default function NotificationsPage() {
  ...
  const list = useNotifications(limit);
  const previous = useRef<Notification[] | undefined>(undefined);
  if (list.data) previous.current = list.data;
  const rows = list.data ?? previous.current;
  const unread = useUnreadCount();
  const markRead = useMarkRead();
  const markAll = useMarkAllRead();
  const count = unread.data ?? rows?.filter((n) => !n.read_at).length ?? 0;
  const unreadKey = (locale === 'is' ? isSingular(count) : count === 1) ? 'notifications.unreadOne' : 'notifications.unread';
  const mayHaveOlder = !!rows && rows.length >= limit;
  const loadingOlder = list.isPending && !!rows;
    ...
    <PageContainer className="max-w-3xl py-6">
      <header className="mb-4 flex flex-wrap items-end justify-between gap-2">
        <div>
          ...
          <p className="text-[13px] text-muted-foreground">{count > 0 ? t(unreadKey, { count }) : t('notifPage.subtitle')}</p>
        </div>
        <Button
          type="button"
          variant="outline"
          ...
          {mayHaveOlder && (
            <div className="mt-3 flex flex-col items-center gap-2">
              ...
              <Button
                ...
                onClick={() => (list.isError ? void list.refetch() : setLimit((n) => n + PAGE_SIZE))}
              >
                {loadingOlder ? t('notifPage.loadingOlder') : list.isError ? t('common.retry') : t('notifPage.loadOlder')}
              </Button>
            </div>
          )}
        </>
