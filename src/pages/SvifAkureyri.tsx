...
export default function SvifAkureyri() {
  ...
  const [selectRow, setSelectRow] = useState<AkureyriRow | null>(null);
  ...
  const patch = (id: string, changes: Partial<AkureyriRow>) =>
    setRows((prev) => prev.map((r) => (r.id === id ? { ...r, ...changes } : r)));

  const toggleValid = async (r: AkureyriRow, notesValue?: string) => {
    ...
    if (error) {
      ...
      toast.success("Valið fyrir Akureyrar-bókina");
    }
  };

  const handleCheck = (r: AkureyriRow) => {
    if (!r.svif_valid) {
      setSelectRow(r);
  ...
  const setOutcome = async (r: AkureyriRow, outcome: string) => {
  ...
  const saveNotes = (r: AkureyriRow, value: string) => {
      ...
      <Dialog open={!!selectRow} onOpenChange={(o) => !o && setSelectRow(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Skrifaðu glósu um fyrirtækið áður en þú byrjar. Fyrirtækið fer í Valin fyrir Akureyrar-bókina.
            </DialogDescription>
          </DialogHeader>
          <Textarea
            value={selectNotes}
