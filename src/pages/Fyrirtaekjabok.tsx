...
export default function Fyrirtaekjabok() {
  ...
  const [selectRow, setSelectRow] = useState<FyrirtaekjabokRow | null>(null);
  ...
  const load = async () => {
    ...
    setRows((data as FyrirtaekjabokRow[]) || []);
    setLoading(false);
  };
  useEffect(() => { load(); }, []);
  ...
  const groups = useMemo(() => {
    const map = new Map<string, FyrirtaekjabokRow[]>();
  ...
  const patch = (id: string, changes: Partial<FyrirtaekjabokRow>) =>
    setRows((prev) => prev.map((r) => (r.id === id ? { ...r, ...changes } : r)));

  const toggleValid = async (r: FyrirtaekjabokRow, notesValue?: string) => {
    const next = !r.svif_valid;
    patch(r.id, { svif_valid: next, ...(notesValue !== undefined ? { notes: notesValue } : {}) });
    const payload: Record<string, unknown> = { svif_valid: next };
    ...
    if (error) {
      ...
      toast.success("Vistað og valið");
    }
  };

  const handleCheck = (r: FyrirtaekjabokRow) => {
    if (!r.svif_valid) {
      setSelectRow(r);
  ...
  const setOutcome = async (r: FyrirtaekjabokRow, outcome: string) => {
  ...
  const saveNotes = (r: FyrirtaekjabokRow, value: string) => {
        ...
        <div className="max-w-5xl mx-auto px-4 py-4 space-y-3">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-muted-foreground">Hringilisti – fyrirtækjabók</p>
            </div>
          </div>
      ...
      <Dialog open={!!selectRow} onOpenChange={(o) => !o && setSelectRow(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Skrifaðu glósu um fyrirtækið áður en þú byrjar. Fyrirtækið verður merkt sem valið.
            </DialogDescription>
          </DialogHeader>
          <Textarea
            value={selectNotes}
